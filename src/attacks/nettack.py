...
class OriginalNettack:
    ...
    Adapted by Lukas Gosch to make applicable to graphs with 
    non-power-law degree distribution.
    """
    ...
    def __init__(self,
        ...
        Args:
            adj (sp.csr_matrix): A without self-loops and normalization.
            X_obs (sp.csr_matrix): -
            z_obs (np.ndarray): -
            W (sp.csr_matrix): -
            u (int): -
    ...
    def attack_surrogate(self, n_perturbations, perturb_structure=True, perturb_features=True, avoid_edge_removal=False,
        ...
        #print(f"Loss before attack: {surrogate_losses[0]:.4f}")
        ...
        for _ in range(n_perturbations):
            if perturb_structure:
                ...
                # Do not consider edges that, if removed, result in singleton nodes in the graph.
                ...
                #print(f"Loss after attack: {best_edge_score:.4f}")
...
class Nettack(LocalAttack):
    ...
    def __init__(self, n_idx: int, X: np.ndarray, A: np.ndarray, y: np.ndarray, 
        ...
        self.sp_attr = sp.csr_matrix(X)
        self.y = y
        if isinstance(attacked_model, DenseGCN):
            assert isinstance(attacked_model.activation, nn.Identity)
            W1 = attacked_model.layers[0][0]._linear.weight.detach().cpu().numpy().T
            W2 = attacked_model.layers[1][0]._linear.weight.detach().cpu().numpy().T
            self.W = sp.csr_matrix(W1.dot(W2))
        else:
    ...
    def create_adversarial_pert(self) -> Tuple[int, int]:
        self.nettack.attack_surrogate(1,
                                      perturb_structure=True,
                                      perturb_features=False,
                                      direct=True)
        return self.nettack.structure_perturbations[-1]